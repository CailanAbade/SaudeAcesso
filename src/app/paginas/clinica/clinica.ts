...
import { Clinica as ClinicaModel } from '../../nucleo/modelos/clinica.model';
...
export class ClinicaComponent {
  ...
  protected preco = inject(Preco);

  clinica = signal<ClinicaModel | null>(null);
  ...
  private carregarClinica(slug: string): void {
    this.catalogo.buscarClinicaPorSlug(slug).subscribe({
      next: (clinica) => {
        if (!clinica) {
          this.carregando.set(false);
          this.naoEncontrada.set(true);
        ...
        forkJoin({
          servicos: this.catalogo.listarServicosPorClinica(clinica.id),
          medicos: this.catalogo.listarMedicosPorClinica(clinica.id),
        }).subscribe({
