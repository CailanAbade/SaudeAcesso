...
} from '../../nucleo/utilitarios/erros-firebase.util';

import {
  TipoUsuario,
} from '../../nucleo/modelos/usuario.model';
...
function senhasIguaisValidator(
  ...
  const senha =
    control.get('senha')?.value;

  const confirmarSenha =
    control.get('confirmarSenha')?.value;
...
export class AutenticacaoComponent {
  ...
  modo =
    signal<'entrar' | 'cadastrar'>(
      'entrar'
    );

  tipoUsuario =
    signal<TipoUsuario>('paciente');
  ...
  selecionarTipo(
    ...
    const controleCpf =
      this.formularioCadastro.get('cpf');

    const controleCnpj =
      this.formularioCadastro.get('cnpj');


    if (tipo === 'paciente') {

      controleCpf?.setValidators([
        Validators.required,
        Validators.minLength(11),
      ]);

      controleCnpj?.clearValidators();

    } else {

      controleCpf?.clearValidators();

      controleCnpj?.setValidators([
        Validators.required,
    ...
    controleCnpj?.updateValueAndValidity();
  }


  alternarVisibilidadeSenha(): void {
    this.senhaVisivel.update(
      (valor) => !valor
    );
  }
  ...
  async aoEnviarEntrar(): Promise<void> {
    ...
    if (
      this.formularioEntrar.invalid
    ) {
      this.formularioEntrar
        .markAllAsTouched();
    ...
    try {
      ...
      const usuario =
        await this.autenticacao.entrar(
          email!,
          senha!
        );
  ...
  async aoEnviarCadastro(): Promise<void> {
    ...
    if (
      this.formularioCadastro.invalid
    ) {
  ...
  private redirecionarAposSucesso(
    usuario: {
      tipo: TipoUsuario;
    } | null
  ): void {
